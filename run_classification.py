...
class DataTrainingArguments:
    ...
    validation_split: Optional[float] = field(
        default=0.3,
        metadata={"help": ("Validation split percentage [0-1].")},
    )
    test_split: Optional[float] = field(
        default=0.1,
        metadata={"help": ("Test split percentage [0-1].")},
    )
...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        # Clean up dataset
        used_columns = data_args.text_column_names.split(",")
        used_columns.append(data_args.label_column_name)
        logger.info(f"Selecting columns: {used_columns}")
        raw_datasets = raw_datasets.select_columns(used_columns)
        logger.info(raw_datasets)
        for column in data_args.text_column_names.split(","):
            raw_datasets = raw_datasets.filter(
                lambda row: row[column] != None and row[column] != ""
            )
        logger.info("Cleaned up dataset.")
        logger.info(raw_datasets)
        # Truncate dataset if needed
        if data_args.max_samples is not None:
            raw_datasets[data_args.train_split_name] = (
                raw_datasets[data_args.train_split_name]
                .shuffle(seed=data_args.shuffle_seed)
                .select(range(data_args.max_samples))
            )
            logger.info("Truncated dataset.")
            logger.info(raw_datasets)
        # Split dataset
        # First split into train and valid+test
        first_split_set = raw_datasets[data_args.train_split_name].train_test_split(
            test_size=data_args.validation_split + data_args.test_split
        )
        # if the test split is not 0,
        if data_args.test_split != 0:
            # then we split valid+test into valid and test
            second_split_set = first_split_set["test"].train_test_split(
                test_size=data_args.test_split
            )
            # merge back to raw_datasets
            raw_datasets[data_args.train_split_name] = first_split_set["train"]
            raw_datasets[data_args.validation_split_name] = second_split_set["train"]
            raw_datasets[data_args.test_split_name] = second_split_set["test"]
        else:
            raw_datasets[data_args.train_split_name] = first_split_set["train"]
            raw_datasets[data_args.validation_split_name] = first_split_set["test"]

        logger.info("Split dataset into train/validate/test split.")
        logger.info(raw_datasets)
